...
if dashboard == 'Trend Predictor':
    ...
    st.markdown(f"<h4 style='text-align: center;'>Uptrend probability for tomorrow: {df['pred_prob'].round(2).values[-1]}, Updated: {df['date'].max().strftime('%Y-%m-%d')} days from today ({future_date.strftime('%Y-%m-%d')}) is: ${predicted_price_on_future_date:.5f},  {((predicted_price_on_future_date-today_price)/today_price)*100:.0f}% difference</h4>", unsafe_allow_html=True)
    fig = go.Figure()
