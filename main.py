...
def plot_risk_visualization(df, instrument):
    ...
    fig.add_trace(go.Scatter(x=df['date'], y=df['Value'], name='Price', line=dict(color='blue', width=3)))
    fig.add_trace(go.Scatter(x=df['date'], y=df['avg'], name='Risk', line=dict(color='yellow', width=3)), secondary_y=True)
